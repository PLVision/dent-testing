...
from dent_os_testbed.utils.test_utils.cleanup_utils import (
    ...
    cleanup_vr_tables as _cleanup_vr_tables,
...
@pytest_asyncio.fixture
async def cleanup_vr_tables(testbed):
    devices = await _get_dent_devs_from_testbed(testbed)
    initial_tables = dict()
    for dev in devices:
        initial_tables[dev.host_name] = await get_initial_tables(dev)
    yield
    table_cleanups = [_cleanup_vr_tables(dev, initial_tables[dev.host_name]) for dev in devices]
