...
async def test_policer_rate_config(testbed, unit_type):
    ...
    streams = tcutil_tc_rules_to_tgen_streams(
        {port_with_rule: out[0][dent]['parsed_output']}, frame_rate_pps=frame_rate, frame_rate_type='line_rate')
    ...
    for unit, mul in unit_multiplier_map[unit_type].items():
        ...
        for row in stats.Rows:
            ...
            assert float(frame_rate - (frame_rate * tolerance)) <= actual_rate <= float(
                frame_rate + (frame_rate * tolerance)), err_msg
