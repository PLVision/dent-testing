...
async def test_policer_rules_priority(testbed, qdisc_type):
    ...
    rule_config = {
        'action': {'police': {'rate': tc_rule_1_frame_rate, 'burst': tc_rule_1_frame_rate + 1000, 'conform-exceed': 'drop'}},
        'want_ip': True,
        'pref': 100,
        'want_port': True,
        'skip_sw': True,
    ...
    # First rule
    tc_rule_1 = tcutil_generate_rule_with_random_selectors(ports_with_rule[0], **rule_config)
    ...
    for row in stats.Rows:
        if qdisc_type == 'port':
            if row['Port Name'] == tg_ports[0]:
                continue
            assert float(tc_rule_1_frame_rate - (tc_rule_1_frame_rate * tolerance)) <= float(
                row['Rx. Rate (bps)']) <= float(tc_rule_1_frame_rate + (tc_rule_1_frame_rate * tolerance))
        else:
            if row['Port Name'] == tg_ports[0]:
                assert float(tc_rule_1_frame_rate - (tc_rule_1_frame_rate * tolerance)) <= float(
                    row['Rx. Rate (bps)']) <= float(tc_rule_1_frame_rate + (tc_rule_1_frame_rate * tolerance))
    ...
    for row in stats.Rows:
        if qdisc_type == 'port':
            if row['Port Name'] == tg_ports[0]:
                continue
            assert float(tc_rule_2_frame_rate - (tc_rule_2_frame_rate * tolerance)) <= float(
                row['Rx. Rate (bps)']) <= float(tc_rule_2_frame_rate + (tc_rule_2_frame_rate * tolerance))
        else:
            if row['Port Name'] == tg_ports[0]:
                assert float(tc_rule_2_frame_rate - (tc_rule_2_frame_rate * tolerance)) <= float(
                    row['Rx. Rate (bps)']) <= float(tc_rule_2_frame_rate + (tc_rule_2_frame_rate * tolerance))
