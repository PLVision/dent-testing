...
async def test_policer_basic(testbed, traffic_type, qdisc_type):
    """
    ...
    3. Create an ingress qdisc on first port connected to Ixia port/shared block and add a rule with random selectors
    ...
    rule_config = {
        ...
        'want_port': True if traffic_type in ['tcp', 'udp'] else None,
        'skip_sw': True,
        'want_vlan': traffic_type == 'l2',
        'want_tcp': traffic_type != 'udp',
        'want_mac': traffic_type == 'l2',
        'want_vlan_ethtype': traffic_type != 'l2',
    }

    tc_rule = tcutil_generate_rule_with_random_selectors(ports_with_rule, **rule_config)

    # Update tc rule protocol based on traffic type
    if traffic_type == 'ipv4':  # forcebly select ipv4
    ...
    for port in ports_with_rule:
        out = await TcFilter.show(input_data=[{dent: [created_rules]}], parse_output=True)
        streams.update(tcutil_tc_rules_to_tgen_streams(
            {port: out[0][dent]['parsed_output']}, frame_rate_type='line_rate', frame_rate_pps=100))
    if traffic_type == 'l2':
        for stream in streams:
            streams[stream]['protocol'] = '0x8100'
    ...
    for row in stats.Rows:
        is_correct_rate = float(frame_rate - (frame_rate * tolerance)) <= float(
            row['Rx. Rate (bps)']) <= float(frame_rate + (frame_rate * tolerance))
