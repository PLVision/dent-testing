...
async def test_ipv4_default_gw(testbed):
    ...
    route_map = {
        ...
        for (port, _, _, def_gw, _, _, _), metric in zip(address_map, (100, 200, None))
    }
    ...
    try:
        ...
        def_route = [route for route in routes if route["dst"] == "default"]
        ...
        for route in routes:
            if route["dst"] != "default":
                continue
            dev = route["dev"]
            err_msg = f"Route {route['gateway']} should have metric {route_map[dev]['metric']}"
            assert "metric" in route and route["metric"] == route_map[dev]["metric"], err_msg
        ...
        for route in routes:
            ...
            if route_map[dev]["should_offload"]:
                ...
                err_msg = f"Route with metric should not be offloaded"
                assert "rt_offload" not in route["flags"], err_msg
                err_msg = f"Route {route['gateway']} should have metric {route_map[dev]['metric']}"
                assert "metric" in route and route["metric"] == route_map[dev]["metric"], err_msg
        ...
        out = await IpRoute.delete(input_data=[{dent: [
            {"dev": route["dev"], "type": "default", "via": route["gateway"]}
            for route in routes if route["dst"] == "default"
        ]}])
        assert out[0][dent]["rc"] == 0, "Failed to remove default routes"

        out = await IpRoute.add(input_data=[{dent: [
            {"dev": route["dev"], "type": "default", "via": route["gateway"]}
            for route in def_route
        ]}])
        assert out[0][dent]["rc"] == 0, "Failed to restore default gw"
