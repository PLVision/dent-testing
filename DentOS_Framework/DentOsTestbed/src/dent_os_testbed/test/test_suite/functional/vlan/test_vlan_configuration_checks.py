...
async def test_vlan_can_set_max_vlans(testbed):
    ...
    # get first port from config
    test_port = dent_devices[0].links_dict[device][1][0]
    max_vlans = 4094
    bridge = 'br0'

    # 2. Create bridge entity and set state to "up" state
    out = await IpLink.add(input_data=[{device: [{'dev': bridge, 'type': 'bridge', 'vlan_filtering': 1}]}])
    assert out[0][device]['rc'] == 0, 'Failed to create bridge'

    out = await IpLink.set(input_data=[{device: [{'dev': bridge, 'operstate': 'up'}]}])
    assert out[0][device]['rc'] == 0, 'Failed to set bridge status to UP'

    # 3. Enslave interface to the created bridge entity
    out = await IpLink.set(input_data=[{device: [{'dev': test_port, 'master': bridge, 'operstate': 'up'}]}])
    ...
    # 5. Verify interface is in all possible (4094) VLANs
    out = await BridgeVlan.show(input_data=[{device: [{'dev': test_port, 'options': '-j'}]}],
                                parse_output=True)
...
async def test_vlan_can_not_add_interface_to_vlan_wo_bridge(testbed):
    ...
    out = await BridgeVlan.add(input_data=[{device: [{'dev': test_port, 'vid': vid}]}])
    assert out[0][device]['rc'] != 0, 'No error on adding interface to VLAN without creating bridge beforehand'

    # 3. Create bridge entity.
    out = await IpLink.add(input_data=[{device: [{'dev': bridge, 'type': 'bridge'}]}])
    assert out[0][device]['rc'] == 0, 'Failed to create bridge'
