...
async def common_bridge_and_igmp_setup(device_name, bridge, igmp_ver, dut_ports, querier_interval=0):
    ...
    out = await IpLink.set(
        input_data=[{device_name: [
            {'dev': bridge, 'type': 'bridge', 'mcast_snooping': 1, 'mcast_igmp_version': igmp_ver}]}])
    ...
    if querier_interval:
        out = await IpLink.set(
            input_data=[{device_name: [
                {'dev': bridge, 'type': 'bridge', 'mcast_querier': 1,
    ...
    out = await IpLink.set(
        input_data=[{device_name: [
            {'dev': port, 'master': bridge, 'operstate': 'up'} for port in dut_ports]}])
...
def verify_expected_rx_rate(stats, tx_port, rx_ports, rate_coef=0, deviation=0.10):
    ...
    exp_rate = float(collected[tx_name]['tx_rate']) * \
        rate_coef if rate_coef else float(collected[tx_name]['tx_rate'])
    for port in rx_ports:
        rx_name = port.split('_')[0]
        res = math.isclose(exp_rate, float(
            collected[rx_name]['rx_rate']), rel_tol=deviation)
...
async def test_igmp_snooping_mixed_ver(testbed, igmp_ver, igmp_msg_ver):
    ...
    macs = ['a6:00:00:00:00:01', 'a6:00:00:00:00:02',
            'a6:00:00:00:00:03', 'a6:00:00:00:00:04']
    ...
    dev_groups = tgen_utils_dev_groups_from_config(
        [{'ixp': tg_ports[0], 'ip': '102.0.0.3', 'gw': '102.0.0.1', 'plen': 24},
         {'ixp': tg_ports[1], 'ip': '100.0.0.3',
             'gw': '100.0.0.1', 'plen': 24},
         {'ixp': tg_ports[2], 'ip': '101.0.0.3',
             'gw': '101.0.0.1', 'plen': 24},
    ...
    assert len(
        router_entires), f'No MDB router entries were added to bridge MDB table {router_entires}'
    ...
    for row in stats.Rows:
        if row['Port Name'] == rx_ports[-1].split('_')[0]:
            assert int(
                row['Bytes Rx. Rate']) <= DEVIATION_BPS, f"Actual rate {row['Bytes Rx. Rate']} expected rate 0"
    ...
    for entry in mdb_entires:
        assert dut_ports[1] != entry[
            'port'], f'Mdb entry of port {dut_ports[1]} still exists: \n{mdb_entires}'
...
async def test_igmp_snooping_diff_source_addrs(testbed):
    ...
    verify_mdb_entries(
        mdb_entires, dut_ports[1:3], mcast_group_addr[:-1], exp=4)
    assert len(
        router_entires), f'No MDB router entries were added to bridge MDB table {len(router_entires)}'
