...
async def test_stp_root_bridge_based_on_mac(testbed, version):
    ...
    # 3. Change the MAC addresses for all bridges
    rc, _ = await dent_device.run_cmd(f'ifconfig {bridge} hw ether 66:DA:78:12:DC:68')
    assert rc == 0, 'Failed to change MAC address'
    ...
    # 7. Change the highest MAC bridge to a MAC with lower value then the root bridge.
    rc, _ = await dent_device.run_cmd(f'ifconfig {bridge} hw ether {get_rand_mac("00:22:XX:XX:XX:XX")}')
    assert rc == 0, 'Failed to change MAC address'
