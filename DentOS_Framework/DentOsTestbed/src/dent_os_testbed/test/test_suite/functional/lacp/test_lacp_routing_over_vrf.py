...
pytestmark = [pytest.mark.suite_functional_lacp,
              ...
              pytest.mark.usefixtures('cleanup_tgen', 'cleanup_bonds',
                                      'cleanup_bridges',  'cleanup_vrf_table_ids', 'cleanup_routes',
                                      'cleanup_vr_tables', 'enable_ipv4_forwarding')]
...
async def test_lacp_routing_over_lag_vrf(testbed):
    ...
    out = await IpLink.add(input_data=[{dent: [{
        'dev': vrf,
        'type': 'vrf',
        'table': id} for vrf, id in vrf_table_ids.items()]}])
    assert out[0][dent]['rc'] == 0, 'Failed to add vrf'

    out = await IpLink.set(input_data=[{dent: [{'device': vrf, 'operstate': 'up'} for vrf in vrf_table_ids]}])
    assert out[0][dent]['rc'] == 0, 'Failed setting vrf to up state'

    out = await IpRoute.add(input_data=[{dent: [{
        'table': id,
        'type': 'unreachable',
        'table_id': 'default'} for id in vrf_table_ids.values()]}])
    ...
    out = await IpLink.set(input_data=[{dent: [{'device': bonds[1], 'master':vrf_1}]}])
    assert out[0][dent]['rc'] == 0, f'Failed enslaving {bonds[1]} to {vrf_1}'

    out = await IpLink.set(input_data=[{dent: [{'device': bonds[2], 'master':vrf_2}]}])
    ...
    out = await IpAddress.add(input_data=[{dent: [{'dev': bonds[2], 'prefix': '2.2.2.1/24'}]}])
    ...
    total_received_bond_2 = sum(
        [int(row['Valid Frames Rx.']) for row in stats.Rows if row['Port Name'] in tgen_lag_2[1]])
    err_msg = f'Expected packets  {tx_packets}, actual packets: {total_received_bond_2}'
    assert isclose(total_received_bond_2, tx_packets / len(tgen_lag_2[1]), rel_tol=0.05), err_msg

    total_received_bond_3 = sum(
        [int(row['Valid Frames Rx.']) for row in stats.Rows if row['Port Name'] in tgen_lag_3[1]])
    err_msg = f'Expected packets  {0.0}, actual packets: {total_received_bond_3}'
    assert isclose(total_received_bond_3, 20, rel_tol=0.2), err_msg

    total_received = sum([int(row['Valid Frames Rx.']) for row in stats.Rows])
    assert isclose(total_received, tx_packets,
                   rel_tol=0.05), f'Expected packets  {total_received}, actual packets: {tx_packets}'
