...
pytestmark = [pytest.mark.suite_functional_lacp,
              pytest.mark.asyncio,
              pytest.mark.usefixtures('cleanup_vr_tables', 'cleanup_tgen', 'cleanup_bonds',
...
async def test_lacp_routing_over_lag_vrf_vlan_device(testbed):
    ...
    Test Overview: Test routing over vrfs
    Test Procedure:
    ...
    4. Configure ip addresses in each LAG and configure route 10.1.1.0/24 via 2 vrf tables
    ...
    dev_groups = tgen_utils_dev_groups_from_config((
        ...
        {'ixp': tgen_lag_3[0], 'ip': '2.2.2.3', 'gw': '2.2.2.1', 'plen': 24, 'lag_members': tgen_lag_3[1]},
        {'ixp': tgen_lag_2[0], 'ip': '3.3.3.4', 'gw': '3.3.3.1', 'plen': 24, 'lag_members': tgen_lag_2[1]}
    ))
    ...
    assert isclose(total_received_bond_3 / len(tgen_lag_2[1]), tx_packets / len(tgen_lag_2[1]), rel_tol=0.05), err_msg
