...
packet_vids = ["x"]  # VLAN tag number contained in the transmitted packet
vlans = {
    2: {
        "ports": [1, 2, 3, 4],
        "pvid": True,
...
@pytest.mark.asyncio
async def test_untagged_broadcast(testbed):
    f"""
    Test Name: Untagged port broadcast
    Test Suite: suite_functional_vlan
    Test Overview: Test broadcast packets can be received on untagged port.
    Test Procedure:
    1.Specify test data.
...
@pytest.mark.asyncio
async def test_untagged_multicast(testbed):
    f"""
    Test Name: Untagged  port multicast
    Test Suite: suite_functional_vlan
    Test Overview: Test multicast packets can be received on untagged port.
    Test Procedure:
    1.Specify test data.
        - packet_vids : {packet_vids}\n
        - vlans\n {vlans}
    ...
    3.Map receiving and non receiving ports.
        Link 1 ->  tx_port
        Links 2-4 -> rx_ports
    3.Setup packet stream for the broadcast packet:
        Packet should be untagged
        src_mac =  ?
    ...
    # 5. Send traffic to rx_ports
    #  Verify traffic received on ports #2 and #3 , #4


@pytest.mark.asyncio
async def test_untagged_unicast(testbed):
    f"""
    Test Name: Untagged port unicast
    Test Suite: suite_functional_vlan
    Test Overview: Test unicast packets can be received on untagged port.
    Test Procedure:
    1.Specify test data.
        - packet_vids : {packet_vids}\n
        - vlans\n {vlans}
    ...
    4.Setup packet stream for the broadcast packet:
        ...
        dst_mac =  ??
    5.Send traffic to rx_ports
        Verify traffic received on port #2 only
    """
