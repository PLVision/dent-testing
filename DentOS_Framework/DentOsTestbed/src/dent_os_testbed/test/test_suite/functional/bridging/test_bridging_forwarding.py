...
async def test_bridging_traffic_from_nomaster(testbed):
    ...
    out = await IpLink.set(
        input_data=[{device_host_name: [{'dev': ports[0], 'nomaster': True}]}])
    assert out[0][device_host_name][
        'rc'] == 0, f"Verify that swp1 entity set to 'nomaster'.\n{out}"
...
async def test_bridging_wrong_fcs(testbed):
    ...
    out = await IpLink.set(
        input_data=[{device_host_name: [
            {'dev': port, 'master': 'br0', 'operstate': 'up'} for port in ports]}])
    err_msg = f"Verify that bridge entities set to 'UP' state and links enslaved to bridge.\n{out}"
    assert out[0][device_host_name]['rc'] == 0, err_msg

    out = await BridgeLink.set(
        input_data=[{device_host_name: [
            {'dev': port, 'learning': True, 'flood': True} for port in ports]}])
