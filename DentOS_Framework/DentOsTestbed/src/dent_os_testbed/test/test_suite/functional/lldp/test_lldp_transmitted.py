...
async def test_lldp_transmitted(testbed):
    """
    Test Name: test_lldp_transmitted
    Test Suite: suite_functional_lldp
    Test Overview: Verify LLDP transmitted packet.
    ...
    7.  Verifying lldp packet captured for swp1.
    """
    ...
    try:
        ...
        assert rc == 0, f'Failed to cat address.\n{out}'

        # wait for the transmitting packets
        await asyncio.sleep(wait)

        # verify LLDP packets count
        await verify_lldp_pkt_count(out, device_host_name, exp_count=4, port=ports[0])
