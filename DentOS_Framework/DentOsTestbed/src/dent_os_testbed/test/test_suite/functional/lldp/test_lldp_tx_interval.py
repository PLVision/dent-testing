...
async def test_lldp_tx_interval(testbed):
    """
    Test Name: test_lldp_tx_interval
    ...
    try:
        ...
        await verify_lldp_pkt_count(out, device_host_name, exp_count=17, port=ports[0])

        # wait for the transmitting packets
        await asyncio.sleep(wait)

        # verify LLDP packets count
        await verify_lldp_pkt_count(out, device_host_name, exp_count=32, port=ports[0])
