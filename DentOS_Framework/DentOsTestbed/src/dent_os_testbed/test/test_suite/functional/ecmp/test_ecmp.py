...
async def verify_ecmp_distribution(stats, tx_port, rx_ports):
    ...
    tx_pkts_sent = [int(row['Frames Tx.'])
                    for row in stats.Rows if row['Port Name'] == tx_port][0]
    rx_pkts_sum = []
    ...
    for row in stats.Rows:
        if row['Port Name'] in rx_ports:
            assert int(row[RX_STATS]) >= expected, p_err_msg.format(
                row['Port Name'], row[RX_STATS], expected)
            rx_pkts_sum.append(int(row[RX_STATS]))
    assert math.isclose(tx_pkts_sent, sum(
        rx_pkts_sum), rel_tol=0.01), rx_sum_msg.format(tx_pkts_sent, sum(rx_pkts_sum))
...
async def test_ecmp_nexthops_down(testbed, nexthops_down):
    ...
    address_map = (
        ...
        (dut_ports[0], tg_ports[0], '1.1.1.1',
         '1.1.1.2',  24, 'aa:bb:cc:dd:ee:11'),
        (dut_ports[1], tg_ports[1], '11.0.0.1',
         '11.0.0.2', 8,  '00:AD:20:B2:A7:75'),
        (dut_ports[2], tg_ports[2], '12.0.0.1',
         '12.0.0.2', 16, '00:59:CD:1E:83:1B'),
        (dut_ports[3], tg_ports[3], '13.0.0.1',
         '13.0.0.2', 20, '00:76:69:89:E0:7B'),
    )
    ...
    stream = {'increment_ip_dst_': {
        ...
        # We dont care about ip_dst as we will use port stats
    ...
    out = await IpLink.set(
        input_data=[{dev_name: [
            {'dev': dut_ports[hop], 'operstate': 'down'} for hop in range(1, nexthops_down + 1)]}])
    ...
    if nexthops_down == 1:
        ...
        for row in stats.Rows:
            if row['Port Name'] == tg_ports[1]:
                assert int(row[RX_STATS]) <= DEVIATION_PKTS, ERR_MSG.format(
                    row['Port Name'], row[RX_STATS])
    else:
        for row in stats.Rows:
    ...
    out = await IpLink.set(
        input_data=[{dev_name: [
            {'dev': dut_ports[hop], 'operstate': 'up'} for hop in range(1, nexthops_down + 1)]}])
    err_msg = f"Verify that ports set to 'UP' state.\n{out}"
    assert not out[0][dev_name]['rc'], err_msg
...
async def test_ecmp_traffic_distribution(testbed, tc_scenario):
    ...
    if tc_scenario == 'nexthop_down':
        out = await IpLink.set(
            input_data=[{dev_name: [
                {'dev': dut_ports[1], 'operstate': 'down'}]}])
        err_msg = f"Verify that ports set to 'DOWN' state.\n{out}"
        assert not out[0][dev_name]['rc'], err_msg
...
async def test_ecmp_distribution_lags(testbed):
    ...
    out = await IpLink.set(
        input_data=[{dev_name: [
            {'dev': port, 'operstate': 'down'} for port in dut_ports[1:]]}])
    ...
    out = await IpLink.set(
        input_data=[{dev_name: [
            {'dev': dut_ports[1], 'master': bond1},
            {'dev': dut_ports[2], 'master': bond2},
            {'dev': dut_ports[3], 'master': bond2}]}])
    ...
    out = await IpLink.set(
        input_data=[{dev_name: [
            {'dev': dev, 'operstate': 'up'} for dev in [dut_ports[0], bond1, bond2]]}])
    ...
    dev_grp = (
        ...
        {'ixp': bond1, 'ip': '11.0.0.2', 'gw': '11.0.0.1',
            'plen': 8, 'lag_members': [tg_ports[1]]},
        {'ixp': bond2, 'ip': '12.0.0.2', 'gw': '12.0.0.1',
            'plen': 16, 'lag_members': tg_ports[2:]}
    )
    address_map = (
        # swp port,    tg port,     swp ip,    tg ip,     plen               lladdr
        (dut_ports[0], tg_ports[0], '1.1.1.1',
         '1.1.1.2',  24, '00:00:00:00:00:10'),
        (bond1,        tg_ports[1], '11.0.0.1',
         '11.0.0.2', 8,  '00:AD:20:B2:A7:75'),
        (bond2,        tg_ports[2:], '12.0.0.1',
         '12.0.0.2', 16, '00:59:CD:1E:83:1B')
    )
    ...
    stream = {'increment_ip_dst_': {
        ...
        'ip_source': dev_groups[tg_ports[0]][0]['name'],
        # We dont care about ip_dst as we will use port stats
        'ip_destination': dev_groups[bond1][0]['name'],
        'srcMac': random_mac(),
        'dstMac': swp_info['mac'],
        'srcIp': f'1.1.1.{randint(3, 100)}',
        'dstIp': {'type': 'increment',
