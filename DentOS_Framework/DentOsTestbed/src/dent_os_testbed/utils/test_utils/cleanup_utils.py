...
async def cleanup_kbyte_per_sec_rate_value(dev):
    """
    Restore all kbyte_per_sec_rate values changed during test.
    ...
    # values by name
    input_data = ({dev.host_name: [
        {"dev": f"{device}", "name": "unk_uc_kbyte_per_sec_rate", "value": "38", "cmode": "runtime"}]}
        for device in devlink_entries['param']
    )
    await DevlinkPort.set(input_data=input_data)
